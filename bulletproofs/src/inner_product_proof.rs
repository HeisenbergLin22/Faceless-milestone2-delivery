...
#[derive(Eq, PartialEq, BorshSerialize, BorshDeserialize, Clone, Debug)]
pub struct InnerProductProof {
    pub(crate) L_vec: Vec<G1>,
    pub(crate) R_vec: Vec<G1>,
    pub(crate) a: Scalar,
    pub(crate) b: Scalar,
}
...
impl InnerProductProof {
    ...
    pub fn create(
        ...
        if n != 1 {
            ...
            let u_inv: Scalar = Option::from(u.inverse()).ok_or(ProofError::FormatError)?;
    ...
    pub(crate) fn verification_scalars(
        ...
        let mut challenges_inv = challenges
            .clone()
            .into_iter()
            .map(|u| Option::from(u.inverse()).ok_or(ProofError::FormatError))
        ...
        IH::Item: Borrow<Scalar>,
    {
        ...
        let expect_P = scalars.zip(points).map(|(s, P)| (*P, s)).collect::<Vec<_>>();
        let expect_P = G1::multiexp(&expect_P);
    ...
    //pub fn from_bytes(slice: &[u8]) -> Result<InnerProductProof, ProofError> {
        ...
        //let mut L_vec: Vec<G1> = Vec::with_capacity(lg_n);
        //let mut R_vec: Vec<G1> = Vec::with_capacity(lg_n);
        //for i in 0..lg_n {
            //let pos = 2 * i * 48;
            //L_vec.push(
...
mod tests {
    ...
    fn test_helper_create(n: usize) {
        ...
        let G: Vec<G1> = bp_gens.share(0).G(n).cloned().collect();
        let H: Vec<G1> = bp_gens.share(0).H(n).cloned().collect();

        // Q would be determined upstream in the protocol, so we pick a random one.
        let Q = G1::hash_to_curve(b"test point", b"tests", &[]);
