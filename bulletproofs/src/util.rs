...
pub fn hash_to_g1(msg: &[u8]) -> G1 {
    ...
    G1::one() * Scalar::new_mul_factor(U256::from_slice(&hash).unwrap())
}

pub fn alt_bn128_g1_sum(v: &[G1]) -> core::result::Result<G1, crate::alt_bn128::AltBn128Error> {
    let sign = vec![false; v.len()];
    let data = sign.iter()
        .zip(v.iter())
        .collect::<Vec<_>>()
        .try_to_vec()
...
pub fn conditional_select<T>(a: T, b: T, choice: u8) -> T {
    if choice == 0 {
        a
    }
    else {
...
pub fn sum_of_powers(x: &Scalar, n: usize) -> Scalar {
    ...
    if n == 0 || n == 1 {
        return Scalar::new_mul_factor(U256::from(n as u64));
    }
    let mut m = n;
