...
use crate::util::{self, alt_bn128_g1_sum};
...
impl<'a, 'b> DealerAwaitingBitCommitments<'a, 'b> {
    ...
    pub fn receive_bit_commitments(
        ...
        let A: G1 = alt_bn128_g1_sum(&bit_commitments.iter().map(|vc| vc.A_j).collect::<Vec<_>>())
            .map_err(|e| MPCError::InvalidG1Aggregate {msg: format!("{:?}", e)})?;
        self.transcript.append_point(b"A", &A);

        let S: G1 = alt_bn128_g1_sum(&bit_commitments.iter().map(|vc| vc.S_j).collect::<Vec<_>>())
            .map_err(|e| MPCError::InvalidG1Aggregate {msg: format!("{:?}", e)})?;
...
impl<'a, 'b> DealerAwaitingPolyCommitments<'a, 'b> {
    ...
    pub fn receive_poly_commitments(
        ...
        let T_1: G1 = alt_bn128_g1_sum(&poly_commitments.iter().map(|pc| pc.T_1_j).collect::<Vec<_>>())
            .map_err(|e| MPCError::InvalidG1Aggregate {msg: format!("{:?}", e)})?;
        let T_2: G1 = alt_bn128_g1_sum(&poly_commitments.iter().map(|pc| pc.T_2_j).collect::<Vec<_>>())
            .map_err(|e| MPCError::InvalidG1Aggregate {msg: format!("{:?}", e)})?;
...
impl<'a, 'b> DealerAwaitingProofShares<'a, 'b> {
    ...
    fn assemble_shares(&mut self, proof_shares: &[ProofShare]) -> Result<RangeProof, ProofError> {
        ...
        let H_factors: Vec<Scalar> = util::exp_iter(
            Option::from(self.bit_challenge.y.inverse()).ok_or(ProofError::FormatError)?,
        )
        .take(self.n * self.m)
        .collect();
