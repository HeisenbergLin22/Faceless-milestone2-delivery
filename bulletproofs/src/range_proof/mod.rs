...
use crate::util;
use crate::util::u64_to_scalar;

use rand::{CryptoRng, RngCore};
use serde::de::Visitor;
use serde::{self, Deserialize, Deserializer, Serialize, Serializer};

pub const FIELD_BYTES_LEN: usize = 32; 
pub const G1_BYTES_LEN: usize = 64; 
pub const G_COMP_BYTES_LEN: usize = 33;
...
impl RangeProof {
    ...
    pub fn prove_single_with_rng<T: RngCore + CryptoRng>(
        ...
        n: usize,
        rng: &mut T,
    ) -> Result<(RangeProof, G1), ProofError> {
    ...
    ) -> Result<(RangeProof, Vec<G1>), ProofError> {
        use self::dealer::*;
        use self::party::*;
    ...
    pub fn verify_multiple_with_rng<T: RngCore + CryptoRng>(
        ...
        pc_gens: &PedersenGens,
        transcript: &mut Transcript,
        value_commitments: &[G1],
        n: usize,
        rng: &mut T,
    ) -> Result<(), ProofError> {
        //let value_commitments: Vec<G1> = value_commitments
            //.iter()
        ...
        let powers_of_2: Vec<Scalar> = util::exp_iter(u64_to_scalar(2u64)).take(n).collect();
        ...
        let y_inv = Option::from(y.inverse()).ok_or(ProofError::FormatError)?;
        let h = s_inv
        ...
        let points = iter::once(self.A.clone())
        ...
        let mega_check = scalars.zip(points)
            .map(|(s, P)| (P, s))
            .collect::<Vec<_>>();
        let mega_check: G1 = G1::multiexp(&mega_check);
    ...
    //pub fn from_bytes(slice: &[u8]) -> Result<RangeProof, ProofError> {
        ...
        //let A = Option::from(G1::deserialize(&read48(&slice[0 * 48..])))
...
fn delta(n: usize, m: usize, y: &Scalar, z: &Scalar) -> Scalar {
    ...
    let sum_2 = util::sum_of_powers(&u64_to_scalar(2u64), n);
