...
impl ProofShare {
    ...
    pub(super) fn audit_share(
        ...
        let y_jn_inv: Scalar = Option::from(y_jn.inverse()).ok_or(())?; // y^(-j*n)
        let y_inv = Option::from(y.inverse()).ok_or(())?; // y^(-1)
        ...
        let P_check = iter::once(Scalar::one())
            ...
            .map(|(s, P)| (*P, s))
            .collect::<Vec<_>>();
        let P_check = G1::multiexp(&P_check);
        if !bool::from(P_check.is_zero()) {
            return Err(());
        }

        let sum_of_powers_y = util::sum_of_powers(&y, n);
        let sum_of_powers_2 = util::sum_of_powers(&u64_to_scalar(2u64), n);
        let delta = (*z - zz) * sum_of_powers_y * y_jn - *z * zz * sum_of_powers_2 * z_j;
        let t_check = iter::once(zz * z_j)
            .chain(iter::once(*x))
            .chain(iter::once(*x * *x))
            ...
            .map(|(s, P)| (*P, s))
            .collect::<Vec<_>>();
        let t_check = G1::multiexp(&t_check);

        if bool::from(t_check.is_zero()) {
            Ok(())
        } else {
            Err(())
        }
