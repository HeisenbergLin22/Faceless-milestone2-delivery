...
use crate::util;
use crate::util::{u64_to_scalar, conditional_select};
...
impl Party {
    ...
    pub fn new<'a>(
        ...
        let V = pc_gens.commit(u64_to_scalar(v), v_blinding);
...
impl<'a> PartyAwaitingPosition<'a> {
    ...
    pub fn assign_position_with_rng(
        ...
        for (G_i, H_i) in bp_share.G(self.n).zip(bp_share.H(self.n)) {
            ...
            let mut point = conditional_select(-(*H_i), *G_i, v_i);
            A = A + point;
            i += 1;
        }
...
impl<'a> PartyAwaitingBitChallenge<'a> {
    ...
    pub fn apply_challenge_with_rng(
        ...
        for i in 0..n {
            let a_L_i = u64_to_scalar((self.v >> i) & 1);
